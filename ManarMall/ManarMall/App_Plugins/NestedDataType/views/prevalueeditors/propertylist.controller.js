function PropertyListController($scope, entityResource, angularHelper) {
    ...
    $scope.add = function () {
        $scope.model.value.push({ alias: '', title: '', descriptoin: '', datatype: 'textstring', required: false, showsummary: false, validation: '' });
    ...
    var setAliasDebounce = _.debounce(function (index) {
        if ($scope.model.value[index].label) {
            $scope.$apply(function () {
                $scope.model.value[index].alias = $scope.model.value[index].label.toLowerCase().toCamelCase().replace(/\W/g, '');
            });
        }
    }, 500);
...
angular.module("umbraco").controller("Umbraco.PrevalueEditors.PropertyListController", PropertyListController);